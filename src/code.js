...
class NepaliNameGenerator {
  static generateName(gender, language, options) {
    ...
    if (
      (options === null || options === void 0 ? void 0 : options.nameType) ===
      "first"
    ) {
      if (language === "nepali") {
...
class NepaliAddressGenerator {
  static generateAddress(provinceFilter) {
    ...
    const province =
      availableProvinces[Math.floor(Math.random() * availableProvinces.length)];
    const district =
    ...
    // Get street name
    const streetName =
      this.streetNames[Math.floor(Math.random() * this.streetNames.length)];
    // Get postal code
...
NepaliAddressGenerator.provinces = [
  {
    name: "Koshi",
    districts: ["Jhapa", "Ilam", "Panchthar", "Taplejung", "Morang", "Sunsari"],
  },
  {
    name: "Madhesh",
    districts: [
  ...
  {
    name: "Lumbini",
    districts: ["Rupandehi", "Kapilvastu", "Dang", "Banke", "Bardiya", "Gulmi"],
  },
  {
    name: "Karnali",
  ...
  {
    name: "Sudurpashchim",
    districts: ["Kailali", "Kanchanpur", "Doti", "Bajhang", "Bajura", "Achham"],
  },
];
  ...
  static generateDate(language, format, includeTime) {
    ...
    if (language === "nepali") {
      ...
      if (format === "long") {
        ...
        return (
          `${nepaliYear} साल ${this.nepaliMonths[month]} ${nepaliDay} गते` +
          (includeTime
            ? `, ` + `${nepaliAmPM} ` + `${nepaliHours}:${nepaliMinutes}`
            : "")
        );
      } else if (format === "short") {
        // Format: "असार १५, २०७८"
        return (
          `${this.nepaliMonths[month]} ${nepaliDay}, ${nepaliYear}` +
          (includeTime
            ? `, ` + `${nepaliAmPM} ` + `${nepaliHours}:${nepaliMinutes}`
            : "")
        );
        ...
        // Format: "२०७८/०४/१५"
        return (
          `${nepaliYear}/${nepaliMonth.padStart(2, "०")}/${nepaliDay.padStart(
            2,
            "०"
          )}` +
          (includeTime
      ...
      if (format === "long") {
        ...
        return (
          `${day} ${this.englishMonths[month]} ${bsYear} BS` +
          (includeTime ? `, ` + `${hours}:${formattedMinutes} ${amPM}` : "")
        );
      } else if (format === "short") {
        // Format: "Ashar 15, 2078"
        return (
          `${this.englishMonths[month]} ${day}, ${bsYear}` +
          (includeTime ? `, ` + `${hours}:${formattedMinutes} ${amPM}` : "")
        );
      } else {
        // Format: "2078/04/15"
        return (
          `${bsYear}/${(month + 1).toString().padStart(2, "0")}/${day
            .toString()
            .padStart(2, "0")}` +
          (includeTime ? `, ` + `${hours}:${formattedMinutes} ${amPM}` : "")
        );
...
NepaliDateGenerator.nepaliNumbers = [
  "०",
  "१",
  "२",
  "३",
    ...
    switch (msg.type) {
      case "generate-name": {
        ...
        if (textNodes.length === 0) {
          ...
          figma.notify(
            `Updated ${textNodes.length} text layer${
              textNodes.length > 1 ? "s" : ""
            }`
          );
        }
        break;
      ...
      case "generate-address": {
        ...
        if (textNodes.length === 0) {
          ...
          // Update all selected text layers
          for (const textNode of textNodes) {
            const address = NepaliAddressGenerator.generateAddress(
              msg.province
            );
            await figma.loadFontAsync(textNode.fontName);
